map.marketV2 <- function (id, area, group, color, scale = NULL, lab = c(group = TRUE, 
                                                        id = FALSE), main = "Map of the Market", print = TRUE) 
{
  ...
  squarified.treemap <- function(z, x = 0, y = 0, w = 1, h = 1, 
    ...
    a <- c(0, cz[1:n])/cz[n]
    if (h > w) {
      viewport.list <- func(viewport.list, 
                            z$label[1:n], 
                            z$color[1:n], 
                            x + w * a[1:(length(a) - 1)], 
                            rep(y,n), x + w * a[-1], 
                            rep(y + h * cz[n], n))
      if (more) {
        viewport.list <- Recall(z[-(1:n), ], x, y + h * cz[n], w, h * (1 - cz[n]), func, viewport.list)
      }
    }
    else {
      viewport.list <- func(viewport.list, z$label[1:n], 
                            ...
                            y + h * a[-1])
      if (more) {
        viewport.list <- Recall(z[-(1:n), ], x + w * cz[n], y, w * (1 - cz[n]), h, func, viewport.list)
      }
    }
    viewport.list
  }
  ...
  for (i in 1:length(group.viewports)) {
    ...
    if (lab[1]) {
      group.tree <- addGrob(group.tree, textGrob(label = group.data$label[i], 
                                                 ...
                                                 check.overlap = T)) # colour of group labels
    }
  ...
  mapmarket <- gTree(name = "MAPMARKET", children = gList(rectGrob(gp = gpar(col = "dark grey",fill = "dark grey"), name = "background"), top.tree,  map.tree))
  if (print) {
    grid.newpage()
    grid.draw(mapmarket)
  }
